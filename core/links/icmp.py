...
class ICMPPacket(Packet):
    ...
    def decode(self):
        if len(self.payload) < 8:
            raise ValueError("ICMP payload must be at least 8 bytes")
        self.type, self.code, self.cksum, self.quench = struct.unpack("!HBBL",
            self.payload)
        self.data = self.payload[8:]

    def encode(self):
        self.payload = struct.pack("!HBBL", self.type, self.code, self.cksum,
            self.quench) + self.data
...
class ICMPLink(Link):
    ...
    def __str__(self):
        if not hasattr(self, "_name"):
            self._name = u"icmplink<%s:%d>" % (self.dest)
        return self._name
...
class UDPLinkClientManager(object):
    ...
    def create(self, callback):
        ...
        self.socket.sendto(struct.pack("!L", MAGIC_WORD), addr)
